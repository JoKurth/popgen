...
autarkify pc = PC.PCL {
    ...
    where
        ...
        transitions = [(MultiSet.fromList [x, x], MultiSet.fromList [x', upState 1]) | i <- [j | j <- [0 .. length initialInput - 1], even j], let x = initialInput !! i, let x' = initialInput !! (i+1)] ++ -- double
                      [(MultiSet.fromList [upState i, upState j], MultiSet.fromList [upState (i+j), upState 0]) | i <- [1 .. numHelpers - 1], j <- [1 .. numHelpers - 1], i+j < numHelpers] ++ -- helper
                      [(MultiSet.fromList [upState i, upState j], MultiSet.fromList [downState numHelpers, upState (i + j - numHelpers)]) | i <- [1 .. numHelpers - 1], j <- [1 .. numHelpers - 1], i+j >= numHelpers] ++
                      [(MultiSet.fromList [downState (i + 1), upState 0], MultiSet.fromList [downState i, hState i]) | i <- [1 .. numHelpers - 1], j <- [1 .. numHelpers - 1]]
